...
Production *getProdStartWith(int symbol) {//when just after dot is non-terminal, this is called
    Production *prodArray[ARRAY_LENGTH(productions)];
    int numMatches = 0;
...
bool isOverlap(int *array[], int symbol) {
    for (int i = 0; i < ARRAY_LENGTH(array); i++) {
        if (*symbolArray[i] == symbol) return false;
    }
    return true;
}
...
int *extractNonTerminal(Production *prodArray[]) {
    ...
    for (int i = 0; i < ARRAY_LENGTH(prodArray); i++) {
        ...
        if (symbol == EXPR || symbol == TERM || symbol == FACTOR && !isOverlap(nonTerminalArray, symbol)) {//categorize non-terminal later
